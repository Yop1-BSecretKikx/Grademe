...
void init_render_traces()
{
    ...
    printf("path : %s",home);
    sprintf(cmd, "mkdir -p \"%s/Desktop/Grademe/render\" \"%s/Desktop/Grademe/traces\"", home, home);
    system(cmd);

    
    sprintf(buff_path, "touch %s/Desktop/Grademe/traces/trace.txt", home);
    system(buff_path);
    }
...
char **fetche_disigned_data(MYSQL *conn, char *target_id)
{
    ...
    {
        printf("\n \033[0;32m[mysql query <<[?ft_name?desc?add_case?expected?]>>]\033[0m\n");
    }
